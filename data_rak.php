...
$result = $conn->query("SELECT * FROM rak ORDER BY id_rak DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<div class="container">
    <h2>Daftar Rak</h2>
    <a href="tambah_rak.php" class="btn">+ Tambah Rak</a>
    <table>
        <tr><th>ID</th><th>Nama Rak</th><th>Aksi</th></tr>
        <?php while($row=$result->fetch_assoc()): ?>
        <tr>
            <td><?= $row['id_rak'] ?></td>
            <td><?= htmlspecialchars($row['nama_rak']) ?></td>
            <td>
                <a href="edit_rak.php?id=<?= $row['id_rak'] ?>" class="btn btn-edit">Edit</a>
                <a href="hapus_rak.php?id=<?= $row['id_rak'] ?>" class="btn btn-danger" onclick="return confirm('Yakin hapus rak ini?')">Hapus</a>
            </td>
        </tr>
        <?php endwhile; ?>
    </table>
